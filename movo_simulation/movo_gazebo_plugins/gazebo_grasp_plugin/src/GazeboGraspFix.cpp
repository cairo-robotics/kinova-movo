...
void GazeboGraspFix::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    ...
    sdf::ElementPtr contactTopicElem = _sdf->GetElement("contact_topic");
    if (!contactTopicElem)
    {
		std::cout<<"GazeboGraspFix: Using  "<<model->GetScopedName()<<" because no <contact_topic> element specified."<<std::endl;
		topic_name = model->GetScopedName();
	} else {
		topic_name = contactTopicElem->Get<std::string>();
		std::cout<<"GazeboGraspFix: Using contact_topic "<<topic_name<<std::endl;
	}

    std::string topic = contactManager->CreateFilter(topic_name, collisionNames);
...
void GazeboGraspFix::OnUpdate() {
    ...
    for (gripCntIt = this->gripCounts.begin(); gripCntIt != this->gripCounts.end(); ++gripCntIt){
	    ...
	    double oldObjDist = relContactPos.Distance(relObjPos);
	    double newObjDist = currLinkWorldPose.Pos().Distance(currObjWorldPose.Pos());
	    double diff = fabs(oldObjDist - newObjDist);
	    /*
            // Get transform for currLinkWorldPose as matrix
            ignition::math::Matrix4<double> worldToLink = ignition::math::Matrix4<double>::Identity;
	    worldToLink.SetRotation(currLinkWorldPose.Rot());
...
double angularDistance(const ignition::math::Vector3<double>& _v1, const ignition::math::Vector3<double>& _v2) {
    ignition::math::Vector3<double> v1=_v1;        
    ignition::math::Vector3<double> v2=_v2;
    v1.Normalize();
    v2.Normalize();
...
void GazeboGraspFix::OnContact(const ConstContactsPtr &_msg)
{
    ...
    for (int i = 0; i < _msg->contact_size(); ++i) {
	    ...
	    ignition::math::Matrix4d worldToLink = ignition::math::Matrix4d(linkWorldPose.Rot());
            ...
            ignition::math::Matrix4<double> worldToObj = ignition::math::Matrix4<double>(objWorldPose.Rot());
